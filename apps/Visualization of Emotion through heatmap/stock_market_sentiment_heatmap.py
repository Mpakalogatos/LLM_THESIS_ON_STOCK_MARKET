...
# Function to dynamically get the headline column
def get_headline_column(news_df):
    if 'Title' in news_df.columns:
        return 'Title'
    elif 'Headlines' in news_df.columns:
        return 'Headlines'
    else:
        return None
...
st.title("📈 Stock News Sentiment Classifier & Visualization")
...
if st.button("Get News and Sentiment Classification"):
    try:
        ...
        if news_df.empty:
            ...
            headline_column = get_headline_column(news_df)

            if headline_column:
                news_df['Title'] = news_df[headline_column].str.lower()  # Use correct column for headlines
                ...
                # Create a mapping of sentiment to numeric values for heatmap
                ...
                # Prepare sentiment pivot table
                sentiment_pivot = news_df.pivot_table(values='Sentiment_Score', index='DateOnly', aggfunc='mean')

                # Plot the heatmap using seaborn
                sns.heatmap(sentiment_pivot.T, annot=True, cmap="coolwarm", cbar_kws={'label': 'Sentiment Score'}, ax=ax)

                # Display the heatmap in Streamlit
                st.pyplot(fig)  # Provide the figure explicitly
